...
if(!isset($_POST['otp']) || empty($_POST['otp'])){
    ob_end_clean();
    header("location: aforgot_verify_otp.php?error=" . urlencode("Please enter OTP"));
    exit;
}
...
if($stmt){
    ...
    if($count > 0){
        ...
        ob_end_clean();
        header("location: aforgot_reset.php");
        exit;
    } else {
        ob_end_clean();
        header("location: aforgot_verify_otp.php?error=" . urlencode("Invalid OTP. Please try again."));
        exit;
    }
} else {
    error_log("Failed to prepare statement for OTP verification");
    ob_end_clean();
    header("location: aforgot_verify_otp.php?error=" . urlencode("Database error. Please try again."));
    exit;
}
?>
